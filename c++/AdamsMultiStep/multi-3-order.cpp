...
vector<double> f(vector<double> y, double h, double x){
    double yp_next = y[1] ;
    double ypp_next = y[2] ;
    double yppp_next = 2*x - 1 - y[0] - y[1] - y[2] ;
    y[0] = yp_next;
    y[1] = ypp_next;
    y[2] = yppp_next;
    return y;
...
vector<double> adams_bashforth(double x0, double y0, double y1, double y2, double h, int n) {
    auto start = high_resolution_clock::now();

    vector<double> y(n+1),y_next(3);
    vector< vector<double> >  y_list { {y0,y1,y2} };
...
vector<double> adams_moulton(double x0, double y0, double y1, double y2, double h, int n) {
...
int main() {
    ...
    for (int i = 0; i <= n; i++) {
        double x = x0 + i*h;
        double y = 2*x + 2*exp(-x) + 2*cos(x) - 3;
        y_exact.push_back(y);
    }

    // Calculate the Adams-Bashforth solution
    vector<double> y_ab = adams_bashforth(x0, y0, y1,y2, h, n);

    // Calculate the Adams-Moulton method
    vector<double> y_am = adams_moulton(x0, y0, y1,y2, h, n);
    ...
    fprintf(gnu, "set title 'Plot of Third-order n=%d'\n",(n)); // กำหนดหัวเรื่องกราฟ
