...
int main()
{
    ...
    for(int i=1;i<=n;i++) {
        double y_next = y + h*yp;
        double yp_next = yp + h*f(x, y, yp);
    ...
    for(int i=1; i<=n;i++) {
        double y_next = y + h*yp;
        double yp_next = yp + h*f(x, y_next, yp);
    ...
    fprintf(gnu, "set title 'Plot of Second-order n=%.0f'\n",(n)); // กำหนดหัวเรื่องกราฟ
    fprintf(gnu, "plot '-' with lines title 'Explicit', '-' with lines title 'y = 1/2*(3sin(x)-xcos(x))','-' with lines title 'Implicit' \n");
